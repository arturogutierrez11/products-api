...
import { OncityOrderResponse, OncityOrderDetail } from '../../../../entities/oncity/orders/OncityOrders';
import { buildOncityHeaders } from '../header/OncityHeaders';
import { mapVtexOrderDetail } from './mapper/mapVtexOrderDetail';
import { IOncityOrdersRepository } from 'src/core/adapters/repositories/orders/orders/IOncityOrdersRepository';
...
export class OncityOrdersRepository implements IOncityOrdersRepository {
  ...
  constructor() {
    this.baseUrl = `https://${process.env.ONCITY_ACCOUNT}.myvtex.com/api/oms/pvt/orders`;
    this.appKey = process.env.ONCITY_APP_KEY!;
    this.appToken = process.env.ONCITY_APP_TOKEN!;
  }

  async getAll(page: number = 1, perPage: number = 15): Promise<OncityOrderResponse> {
    try {
      ...
      console.error('Error fetching On city orders:', error.response?.data || error.message);
  ...
  async getById(orderId: string): Promise<OncityOrderDetail> {
    try {
      ...
      console.error(`Error fetching On city order ${orderId}:`, error.response?.data || error.message);
